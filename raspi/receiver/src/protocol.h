...
#define PROTOCOL_DYNAMIC_BUFFER_SIZE 16 + 2
...
typedef struct __attribute__((__packed__)) {
	CC110L_RSSI_BYTE rssi;
	CC110L_CRC_REG_BYTE crc;
} PROTOCOL_RECEIVE_APPEND;
...
typedef struct __attribute__((__packed__)) {
	...
	uint16_t	si1147_vis;
	uint16_t	si1147_uv;
	PROTOCOL_RECEIVE_APPEND append;
...
typedef struct __attribute__((__packed__)) {
	uint16_t 	hdc1000_temp;
	uint16_t 	hdc1000_humi;
	uint16_t 	tmp102_temp;
	uint16_t 	si1147_ir;
	uint16_t	si1147_vis;
	uint16_t	si1147_uv;
	PROTOCOL_RECEIVE_APPEND append;
